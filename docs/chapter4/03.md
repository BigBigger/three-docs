...
你可以看到这个渲染循环每执行一次，time 变量就会增加 `0.01` 。这个信息会传递给 `vertexShader` ，用来计算方块顶点的新位置。现在打开例子 [ `08-shader-material.html` ](/example/chapter4/08-shader-material) ，你会看到这个方块沿着它的轴收缩、膨胀，如图 4.17 所示。
