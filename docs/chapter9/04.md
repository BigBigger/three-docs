...
为了能够让动画运行起来，我们还需要创建 `THREE.SkinnedMesh` 对象，就前一个例子所做的，共同使用 `THREE.AnimationMixer` 、 `THREE.AnimationClip` 和 `THREE.ClipAction` 告诉 Three.js 如何运行导入的动画。要记住在渲染循环中必须调用 `mixer.update()` 函数。这个示例（ [ `13-animation-from-blender.html` ](/example/chapter9/13-animation-from-blender) ）展示的是正在动作的手，效果如图 9.21 所示。
...
上面代码使用了标准材质 `THREE.MeshStandardMaterial` 来为模型添加皮肤。并且基于加载获得的动画数据，创建了三个动画对象。由于该模型中没有骨骼信息，因此 `morphTargets` 属性被设置为 `true` ，以便启用变形动画模式。示例程序 [ `15-animation-from-md2.html` ](/example/chapter9/15-animation-from-md2) 有一个选择动画的下拉菜单，可以在菜单中选择播放模型提供的所有动画，如图 9.24 所示。
...
示例程序 [ `chapter-09/18-animation-fromsea.html` ](/example/chapter9/18-animation-fromsea) 的截图如图 9.30 所示。
