...
const matterService = require('./frontmatter-service');
const workspacePath = path.resolve(__dirname, '..', '..');

const h1MdRegExp = /^#\s+(.+)\s+(\{#([\w-]+)\})$/;
/** 在此书写所有文章所在的目录名 */
const articleDirs = ['docs'];
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (stats.isFile()) {
        ...
        if (articleDirs.some((dir) => filePath.split(path.sep).includes(dir)))
...
module.exports = async () => await ergodicDirectory(workspacePath);
